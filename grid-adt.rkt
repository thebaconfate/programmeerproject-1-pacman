...
(define (make-grid-adt dim-i dim-j)
  (let ((grid (make-vector dim-i)))
    ...
    (define (primitive? value)
      (or (number? value) (boolean? value) (symbol? value) (string? value)))

    (define read-grid (lambda (i j) (vector-ref (vector-ref grid i) j)))

    (define write-grid! (lambda (i j new-value) (vector-set! (vector-ref grid i) j new-value)))
